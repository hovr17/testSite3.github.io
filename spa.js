...
class SPAController {
    constructor() {
        // Отключаем старый PageTransitionManager если он существует
        if (window.pageTransitionManager) {
            console.log('⚠️ Отключаем старый PageTransitionManager для SPA');
            window.pageTransitionManager = null;
        }
    ...
    async performTransition(template, direction) {
        ...
        // Определяем анимацию в зависимости от устройства
